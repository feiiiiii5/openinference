...
**Status:** accepted 2026-07. Ships as a media-agnostic **`BlobUploader` contract** plus
an **image offload policy** in `openinference-instrumentation`. No packaged uploader
implementation ([follow-up](#5-follow-up-a-packaged-general-purpose-uploader)) and no semantic-convention changes.
...
- **Design:** a pluggable **`BlobUploader`** — decoded bytes go to external storage
  ...
  new value: `...message_content.image.image.url` = `"s3://…/3a7bd3….png"` instead
  of `"__REDACTED__"`.
...
- **`shutdown(timeout_sec)`** flushes pending work and stops workers. Nothing in the
  core package calls it — the application (or SDK) that constructs the uploader owns
  its lifecycle; a packaged implementation would wire it to `atexit` ([follow-up](#5-follow-up-a-packaged-general-purpose-uploader)).
...
validated against the protocol. Resolution failures (unknown name, wrong type, import
error) log a warning and leave the uploader unset — misconfiguration degrades to
redaction, never crashes. The uploader's *own* configuration (bucket, credentials) is
its own concern, typically read from its own env vars, keeping fully zero-code
...
or anything raises. Any instrumentor that uses `OITracer` — which the project
requires — gets image offload without a diff.
...
| process exits mid-queue | implementation's `shutdown()` flush; still-pending blobs may be lost | same as above |
...
unchanged, and Phoenix already renders URL-valued `image.image.url`. Resolution
(signed URLs, proxying, retention) is the storage backend's concern, outside this
design.
...
One live script, [`scripts/image_blob_demo.py`](./scripts/image_blob_demo.py)
(`uv run --script …` against a local Phoenix, `OPENAI_API_KEY` required): one
chat-completions vision request through the released OpenAI auto-instrumentor, run
twice with only the `TraceConfig` changed — redaction vs. storage URI on the same
attribute key. The script resolves `openinference-instrumentation` from **this repo**
via a `[tool.uv.sources]` path in its PEP 723 metadata, so it exercises the shipped
`Blob`/`BlobUploader`/`TraceConfig` code rather than a vendored copy. The demo blob
store (`LocalBlobStore`, inlined in the script) is a deliberate mock —
content-addressed local files — precisely because OpenInference ships no uploader.
